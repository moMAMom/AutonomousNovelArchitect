...
│   ├── inception_dialog.py     # 新規プロジェクト作成ダイアログ（単一テキスト入力）
...
└── output/
    └── {project_name}/
        ...
        └── manuscript/
            ├── ch{N:02d}_final.txt
            └── logs/
                ├── ch{N:02d}_draft_{attempt:02d}.txt   # Drafting: 各試行の原稿
                ├── ch{N:02d}_critique_{attempt:02d}.txt # Critique: スコア+改善点
                ├── ch{N:02d}_polished.txt               # Polishing: 校閲後の稿
                ├── ch{N:02d}_drafts.jsonl               # 全試行の構造化ログ
                └── guard_actions.jsonl
```
...
### 6.2 Inception ダイアログ（`inception_dialog.py`）

`QDialog` 単一画面構成：

| ウィジェット | 説明 |
|---|---|
| プロジェクト名（QLineEdit, 必須） | 出力ディレクトリ名にも使用 |
| 目標章数（QSpinBox, 5〜20, default 10） | ロック対象 |
| 作品概要テキスト枠（QTextEdit, 大） | ジャンル・世界観・主人公・登場人物・テーマ・起承転結ヒントを自由記述またはコピペで入力 |
| 「テンプレートをコピー」ボタン | フォーマット文字列をクリップボードへコピー。ChatGPT 等で原案を作成して貼り付ける運用を想定 |

`get_data()` 戻り値: `{ project_name, target_chapters, concept }`  
`concept` はそのまま `start_inception()` の `keywords["concept"]` として渡す。

「OK」→ `Orchestrator.start_inception()` 呼び出し → `project_bible.json` 生成 → BibleViewer で確認（この時点ではロックなし・手動編集可）。
...
| `finished_chapter` | `int` | 1 章完了（OrchestratorWorker が emit） |

---

## 9. ノンストップモード

**原則:** テンプレート入力 → OK → 全章完成まで無停止自動実行。

### 9.1 自動連鎖フロー

```
InceptionDialog.OK
  → start_inception() + start_structuring()   ← バックグラウンドスレッド
  → _on_inception_done()
      → _run()  ← 自動開始（第1章）
  → _on_chapter_finished(chapter_id)
      → 未完了章があれば _start_chapter_worker(next_ch)  ← 自動連鎖
      → なければ「全章完成」表示
```

### 9.2 中断・再開

- ツールバー「■ 停止」→ `request_stop()` で現在の章の処理後に停止。
- 再起動後に「▶ 実行」→ `progress.json` から resume して未完了章から再開。
- `chapter_approval_gate: true` に設定した場合のみ、各章の承認待ちで停止する。

### 9.3 各段階の出力ファイル（Recursive Drafting）

各章の `manuscript/logs/` に以下を出力する：

| ファイル名 | 内容 | タイミング |
|---|---|---|
| `ch{N:02d}_draft_{attempt:02d}.txt` | Writer が生成した生ドラフト | Drafting 完了後 |
| `ch{N:02d}_critique_{attempt:02d}.txt` | スコア・サマリー・改善箇条書き | Critique 完了後 |
| `ch{N:02d}_polished.txt` | Proofreader 済みの最終候補稿 | Polishing 完了後 |
| `ch{N:02d}_drafts.jsonl` | 全試行の構造化ログ（JSON Lines） | 各試行後（既存） |
| `ch{N:02d}_final.txt` | Archiving 後の確定稿 | Archiving 完了後（既存） |
| `guard_actions.jsonl` | Guard の Bible 更新記録 | Archiving 時（既存） |
